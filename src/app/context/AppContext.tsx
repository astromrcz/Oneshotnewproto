...
export function AppProvider({ children }: { children: ReactNode }) {
 ...
 const [staffProfile, setStaffProfile] = useState<StaffProfile>(() => {
  ...
  // 🟢 FIXED: Cloud Backup Helper with 1.5s delay to completely eliminate SQLite race conditions
  ...
  const assignTable = (tableId: string, session: Session) => {
    ...
    // Direct Cloud Update for TV Display
  ...
  const addSessionHistory = (i: Omit<SessionHistoryItem, 'id'>) => {
    ...
    // Direct cloud push to guarantee Analytics accuracy
 ...
 const resetStaffUserPassword = async (id: string) => {
  ...
  const updateStaffProfile = (p: Partial<StaffProfile>) => {
    setStaffProfile(prev => {
      ...
      syncToDB(`/api/staff/${updated.username}`, 'PUT', p, `Update profile`).then(runCloudBackup).catch(()=>{});
      return updated;
    });
  };
